...
public class MapFragment extends Fragment implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener, GoogleMap.OnMapClickListener {
    ...
    private static final int UPDATE_INTERVAL_MS = 1000;  // 1Ï¥?
    private static final int FASTEST_UPDATE_INTERVAL_MS = 500; // 0.5Ï¥?
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        search_btn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    AddressList = geocoder.getFromLocationName(str, 10); // ?ñª?ñ¥?ò¨ Í∞íÏùò Í∞úÏàò
                } catch (IOException e) {
                    e.printStackTrace();
                }
                if (AddressList != null) {
                    if (AddressList.size() == 0) {
                        System.out.println("Ï£ºÏÜå ?ò§Î•?");
                    } else {
                        System.out.println(AddressList.get(0).toString());
                        String[] splitStr = AddressList.get(0).toString().split(",");
                        String address = splitStr[0].substring(splitStr[0].indexOf("\"") + 1, splitStr[0].length() - 2); // Ï£ºÏÜå
                        System.out.println(address);
                        String latitude = splitStr[10].substring(splitStr[10].indexOf("=") + 1); // ?úÑ?èÑ
                        String longitude = splitStr[12].substring(splitStr[12].indexOf("=") + 1); // Í≤ΩÎèÑ
...
//                Postc.RemoveMarker(x1, x2, y1, y2);
        ...
        tabLayout.addTab(tabLayout.newTab().setText("?òÑ?û¨ Pick"));
        tabLayout.addTab(tabLayout.newTab().setText("Í±∞Î¶¨?àú"));
        tabLayout.addTab(tabLayout.newTab().setText("Í∞?Í≤©Ïàú"));
        ...
        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                if (pos == 0) {
                    System.out.println("???ÎßàÏª§---------------------------------------------------------");
                    ZoomLevel = mMap.getCameraPosition().zoom;
                    lat = mMap.getCameraPosition().target.latitude;
                    lng = mMap.getCameraPosition().target.longitude;
                    // Ï§åÎ†àÎ≤®Ïù¥ 11, 13, 15?ùº?ïå ?Üµ?ã†
                    System.out.println("?Üµ?ã†---------------------------------------------------------");
                    x1 = (lat + 1 * (0.012 * (2 ^ (int) (15.0 - ZoomLevel))));
                    y1 = (lng - 1 * (0.012 * (2 ^ (int) (15.0 - ZoomLevel))));
                    x2 = (lat - 1 * (0.012 * (2 ^ (int) (15.0 - ZoomLevel))));
                    y2 = (lng + 1 * (0.012 * (2 ^ (int) (15.0 - ZoomLevel))));
                    Postc.send_Location(x1, y1, x2, y2);
                    System.out.println(ZoomLevel + " ?úÑÏπ? :  (" + x1 + ", " + y1 + ")" + " (" + x2 + ", " + y2 + ")");
                    tab1_layout.setVisibility(View.VISIBLE);
                    tab2_layout.setVisibility(View.GONE);
                } else if (pos == 1) {

                    System.out.println("Í±∞Î¶¨?àú---------------------------------------------------------");
                    ZoomLevel = mMap.getCameraPosition().zoom;
                    lat = mlatitude;
                    lng = mlongitude;
                    // Ï§åÎ†àÎ≤®Ïù¥ 11, 13, 15?ùº?ïå ?Üµ?ã†
    ...
    private void getData_pick(String Address, int Price, double distance) {
        ...
        List<String> listContent_Price = Arrays.asList("Ï£ºÏ∞®?öîÍ∏? : " + "?ãúÍ∞ÑÎãπ " +Price + "?õê");
        List<String> listContent_Time = Arrays.asList("?ö¥?òÅ?ãúÍ∞? :00:00 ~ 12:00");
        ...
        for (int i = 0; i < listAddress.size(); i++) {
            // Í∞? List?ùò Í∞íÎì§?ùÑ data Í∞ùÏ≤¥?óê set ?ï¥Ï§çÎãà?ã§.
            Data data = new Data();
            ...
            // Í∞? Í∞íÏù¥ ?ì§?ñ¥Í∞? dataÎ•? adapter?óê Ï∂îÍ???ï©?ãà?ã§.
            adapter.addItem(data);
        }

        // adapter?ùò Í∞íÏù¥ Î≥?Í≤ΩÎêò?óà?ã§?äî Í≤ÉÏùÑ ?ïå?†§Ï§çÎãà?ã§.
    ...
    private void getData_distance() {
        ...
        for(int i = 0; i < Selected_Parking.length; i++) {
            if((int) getDistance(mlatitude, mlongitude, latlngConv(Double.parseDouble(Selected_Parking[i][2]), Double.parseDouble(Selected_Parking[i][3]))) < 6000) {
                listadapter.addItem(Selected_Parking[i][0], Selected_Parking[i][1] + " ?õê", (int) getDistance(mlatitude, mlongitude, latlngConv(Double.parseDouble(Selected_Parking[i][2]), Double.parseDouble(Selected_Parking[i][3]))) + "m");

               System.out.println(Selected_Parking[i][0] + ", " + Selected_Parking[i][1] + " ?õê"+ ", " + Selected_Parking[i][2]+ ", " +Selected_Parking[i][3]);
            }
        }
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //?ï°?ã∞ÎπÑÌã∞Í∞? Ï≤òÏùå ?Éù?Ñ±?ê† ?ïå ?ã§?ñâ?êò?äî ?ï®?àò
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        clusterManager.setOnClusterItemClickListener(new ClusterManager.OnClusterItemClickListener<MyItem>() {
            ...
            public boolean onClusterItemClick(MyItem item) {
                ...
                System.out.println("Ï£ºÏÜå : "+ getAddress(item.getPosition().latitude, item.getPosition().longitude));
                init();
